...
export const JoinPool = (props: any) => {
    ...
    const command = joinPoolCommand ?
        joinPoolCommand : 'curl -sSf -L https://1to.sh/join | sudo sh';
    ...
    function UpDownSwitch() {
        return (
            <div className='flex text-default items-center cursor-pointer select-none' onClick={() => {
        ...
        return (
            <div className='mt-4 text-sm text-default font-medium'>
                {!joinPoolCommand && <>
                    ...
                    and{' '}
                    <span className='text-white'>address</span>, it appears on console and will be saved on your disk.
                    If you already have an address —{' '}
                    <span className='text-white'>type it above</span>{' '}
                    for autofill.<br />
                </>}
